...
import "./AirRingDrillingChart.css";
import 'font-awesome/css/font-awesome.min.css';
import {QRCodeSVG} from "qrcode.react";
import BASE_URL from "../config";
import {useNavigate, useParams} from "react-router-dom";

const AirRingDrillingChart = () => {
    const {component} = useParams();
    const [airRingDrillingData, setAirRingDrillingData] = useState([]);
...
// Fetch and organize AirRing data
        ...
        try {
            ...
            const internalDiameterMap = internalDiameters.reduce((acc, curr) => {
                acc[curr.record_sr_no] = {
                    ...
                    runits: curr.runits,
                    jet_diameter: curr.jet_diameter,
                    tap: curr.tap,
                    cdx: curr.cdx,
                    FinishSize: curr.FinishSize, 
                };
                return acc;
            }, {});
            ...
            const mergedData = rawData.map((entry) => {
                ...
                return {
                    ...
                    tap: sizeDetails.tap || "N/A",
                    cdx: sizeDetails.cdx || "N/A",
                    FinishSize: sizeDetails.FinishSize || "N/A",
                };
            });
            ...
            setAirRingDrillingData(dataArray);
    ...
    const handleSearch = (e) => {
        ...
        if (query === "") {
            setFilteredData(airRingDrillingData);
        } else {
            setFilteredData(
                airRingDrillingData.filter((entry) =>
    ...
    return (
        <div className="airring_drilling_data_entry__container">
            <div className="airring_drilling_table_container">
                <div className="drilling-work-order-range-container">
                    {!isDateRangeSelected && (
                        <div className="drilling-work-order-range-section">
                            <button className="back-button" onClick={() => navigate(-1)}>⬅ Back</button>
                            <h2 className="drilling-work-order-range-title1">Air Ring
                                Drilling
                                Chart</h2>
                            <h3 className="drilling-work-order-range-title2">Select Work Order
                                Range</h3>
                            <div className="drilling-work-order-range-inputs">
                    ...
                    <div className="airring_drilling_search-container">
                        ...
                        {isDateRangeSelected && (
                            <div>
                                ...
                                />
                                <div className="airring_drilling_table-container">
                                <button className="chart-back-button" onClick={() => navigate(-1)}>⬅ Back</button>
                                    <button onClick={handlePrint} className="drilling_data_entry_button">
                                        Print Report
                                    </button>
                                    <div className="airring_drilling_data_container" ref={printRef}>
                                        <table className="airring_drilling_data_entry__table">
                                            <thead>
                                            <tr className="h-title">
                                            <img src="/LogoNew.png" style={{width: "0px"}} alt=""></img>
                                            <th colSpan="14"
                                                    className="airring_drilling_data_table_title_1 title_1">
                                                    Quality Management System
                                                    <br/>
                                                    AirRing: Drilling Chart
                                                </th>
                                            </tr>
                                            <tr className="t-head">
                                                <th className="g-border blank work">Work Order No</th>
                                                <th className="g-border">SUB SR NO.</th>
                                                <th className="g-border">QTY</th>
                                                <th className="g-border Id">UNITS</th>
                                                <th className="g-border">FINISH DIA.</th>
                                                <th className="g-border">JET TYPE</th>
                                                <th className="g-border blank">JET COUNT</th>
                                                <th className="g-border">CDX</th>
                                                <th className="g-border">TAP</th>
                                                <th className="g-border">REMARK</th>
                                                <th className="g-border Id">COMPLETE DATE</th>
                                                <th className="g-border Id">TENTATIVE DISPATCH DATE</th>
                                                <th className="g-border">QR Codes</th>
                                            </tr>
                                            </thead>
                                            <tbody>
                                            {isTableVisible && (
                                                                            ...
                                                                            >
                                                                                ...
                                                                                {shouldPrintWorkOrder && (
                                                                                    <td
                                                                                        ...
                                                                                        className={`airring_drilling_party-name-cell ${isLastEntryInParty ? 'no-bottom-border' : ''}`}
                                                                                        style={{
                                                                                    ...
                                                                                    >
                                                                                        <strong
                                                                                            className="airring_drilling_p_span">
                                                                                ...
                                                                                {!shouldPrintWorkOrder && entryIndex === 0 && (
                                                                                    <td
                                                                                        ...
                                                                                        className="airring_drilling_party-name-cells"
                                                                                        style={{
                                                                                ...
                                                                                <td className="g-border">{entry.sub_sr_no}</td>
                                                                                {/* Display other fields */}
                                                                                <td className="g-border">{entry.qty}</td>
                                                                                <td className="g-border u">{entry.units}</td>
                                                                                {/* <td className="g-border u">{entry.units}</td> */}
                                                                                <td className="g-border blank Id">{entry.FinishSize}</td>
                                                                                <td className="g-border">{entry.bore_jet_type}</td>
                                                                                <td className="g-border">{entry.jet_count}</td>
                                                                                <td className="g-border">{entry.cdx}</td>
                                                                                <td className="g-border">{entry.tap}</td>
                                                                                <td className="g-border rmark">{entry.remark}</td>
                                                                                <td className="g-border blank Id">{entry.complete_date || "__/__/__"}</td>
                                                                                <td className="g-border Id">{entry.t_dispatch_week}</td>
...
export default AirRingDrillingChart;