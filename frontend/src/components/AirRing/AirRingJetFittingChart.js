...
import "./AirRingJetFittingChart.css";
import 'font-awesome/css/font-awesome.min.css';
import { QRCodeSVG } from "qrcode.react";
import BASE_URL from "../config";
import { useNavigate, useParams } from "react-router-dom";

const AirRingJetFittingChart = () => {
    ...
    const fetchInternalDiameters = async () => {
        try {
            const response = await axios.get(`${backendUrl}/internal-diameter`);
...
const fetchDailyData = useCallback(async (fromWorkOrderNo, toWorkOrderNo) => {
    try {
        let url = `${backendUrl}/airring-history`;
        ...
        const internalDiameterMap = internalDiameters.reduce((acc, curr) => {
            acc[curr.record_sr_no] = {
                ...
                FinishSize: curr.FinishSize, // Include FinishSize
                size: curr.size,
            };
            return acc;
        }, {});
        ...
        const mergedData = rawData.map((entry) => {
            ...
            let internalDiameterValue;

            // Check conditions for size and units
            if (sizeDetails.size === "M" && 
                ["BAKER MERCER MODULE 1A +/-0.020", "BAKER MERCER MODULE 1A +/-0.040", "BAKER MODULE 2A"].includes(sizeDetails.units)) {
                internalDiameterValue = sizeDetails.FinishSize || 'N/A'; // Use FinishSize if conditions are met
            } else {
                internalDiameterValue = sizeDetails.baker_type === 'baker' 
                    ? (sizeDetails.baker_internal_diameter || 'N/A') 
                    : (sizeDetails.non_baker_internal_diameter || 'N/A');
            }

            return {
                ...entry,
                internal_diameter: internalDiameterValue, // Assign the calculated internal diameter value
                ...
                units: sizeDetails.units || "N/A",
                jet_diameter: sizeDetails.jet_diameter || "N/A",
                FinishSize: sizeDetails.FinishSize || "N/A", // Include FinishSize
                size: sizeDetails.size || "N/A" // Ensure size is included
            };
        });
    ...
    return (
        <div className="airring_jetfitting_data_entry__container">
            <div className="airring_jetfitting_table_container">
                <div className="jetfitting-work-order-range-container">
                    {!isDateRangeSelected && (
                        <div className="jetfitting-work-order-range-section">
                            <button className="back-button" onClick={() => navigate(-1)}>⬅ Back</button>

                            <h2 className="jetfitting-work-order-range-title1">Air Ring
                                Jet Fitting
                                Chart</h2>
                            <h3 className="jetfitting-work-order-range-title2">Select Work Order
                                Range</h3>
                            <div className="jetfitting-work-order-range-inputs">
                    ...
                    <div className="airring_jetfitting_search-container">
                        ...
                        {isDateRangeSelected && (
                            <div>
                                ...
                                <div className="airring_jetfitting_table-container">
                                    <button className="chart-back-button" onClick={() => navigate(-1)}>⬅ Back</button>

                                    <button onClick={handlePrint} className="jetfitting_data_entry_button">
                                        Print Report
                                    </button>
                                    <div className="airring_jetfitting_data_container" ref={printRef}>
                                        <table className="airring_jetfitting_data_entry__table">
                                            <thead>
                                            <tr className="h-title">
                                            <img src="/LogoNew.png" style={{width: "0px"}} alt=""></img>
                                            <th colSpan="14"
                                                    className="airring_jetfitting_data_table_title_1 title_1">
                                                    Quality Management System
                                                    <br/>
                                                    AirRing: Jet Fitting Chart
                                                </th>
                                            </tr>
                                            <tr className="t-head">
                                                ...
                                                <th className="gborder">JET DIA.</th>
                                                                            ...
                                                                            >
                                                                                ...
                                                                                {shouldPrintWorkOrder && (
                                                                                    <td
                                                                                        ...
                                                                                        className={`airring_jetfitting_party-name-cell ${isLastEntryInParty ? 'no-bottom-border' : ''}`}
                                                                                        style={{
                                                                                    ...
                                                                                    >
                                                                                        <strong
                                                                                            className="airring_jetfitting_p_span">
                                                                                ...
                                                                                {!shouldPrintWorkOrder && entryIndex === 0 && (
                                                                                    <td
                                                                                        ...
                                                                                        className="airring_jetfitting_party-name-cells"
                                                                                        style={{
                                                                                ...
                                                                                <td className="gborder Id">
                                                                                    {entry.size === "M" || 
                                                                                    ["BAKER MERCER MODULE 1A +/-0.020", "BAKER MERCER MODULE 1A +/-0.040", "BAKER MODULE 2A "].includes(entry.units) 
                                                                                    ? (entry.FinishSize || 'N/A') 
                                                                                    : (entry.baker_type === 'baker' ? (entry.baker_internal_diameter || 'N/A') : (entry.non_baker_internal_diameter || 'N/A'))}
                                                                                </td>
                                                                                <td className="gborder">{entry.bore_jet_type}</td>
                                                                                <td className="gborder blank">{entry.units}</td>
                                                                                <td className="gborder Id">{entry.jet_diameter}</td>
...
export default AirRingJetFittingChart;