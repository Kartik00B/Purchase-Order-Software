...
import "./AirRingInspectionChart.css";
import 'font-awesome/css/font-awesome.min.css';
import {QRCodeSVG} from "qrcode.react";
import BASE_URL from "../config";
import {useNavigate, useParams} from "react-router-dom";

const AirRingInspectionChart = () => {
    const {component} = useParams();
    const [airRingInspectionData, setAirRingInspectionData] = useState([]);
...
// Fetch and organize AirRing data
    const fetchDailyData = useCallback(async (fromWorkOrderNo, toWorkOrderNo) => {
        try {
            ...
            setAirRingInspectionData(dataArray);
    ...
    const handleSearch = (e) => {
        ...
        if (query === "") {
            setFilteredData(airRingInspectionData);
        } else {
            setFilteredData(
                airRingInspectionData.filter((entry) =>
    ...
    return (
        <div className="airring_inspection_data_entry__container">
            <div className="airring_inspection_table_container">
                <div className="inspection-work-order-range-container">
                    {!isDateRangeSelected && (
                        <div className="inspection-work-order-range-section">
                        <button className="back-button" onClick={() => navigate(-1)}>⬅ Back</button>

                            <h2 className="inspection-work-order-range-title1">Air Ring
                                Inspection
                                Chart</h2>
                            <h3 className="inspection-work-order-range-title2">Select Work Order
                                Range</h3>
                            <div className="inspection-work-order-range-inputs">
                    ...
                    <div className="airring_inspection_search-container">
                        ...
                        {isDateRangeSelected && (
                            <div>
                                ...
                                <div className="airring_inspection_table-container">
                                <button className="chart-back-button" onClick={() => navigate(-1)}>⬅ Back</button>

                                    <button onClick={handlePrint} className="inspection_data_entry_button">
                                        Print Report
                                    </button>
                                    <div className="airring_inspection_data_container" ref={printRef}>
                                        <table className="airring_inspection_data_entry__table">
                                            <thead>
                                            <tr className="h-title">
                                            <img src="/LogoNew.png" style={{width: "0px"}} alt=""></img>

                                                <th colSpan="14"
                                                    className="airring_inspection_data_table_title_1 title_1">
                                                    Quality Management System
                                                    <br/>
                                                    AirRing: Inspection Chart
                                                </th>
                                            </tr>
                                            <tr className="t-head">
                                                                            ...
                                                                            >
                                                                                ...
                                                                                {shouldPrintWorkOrder && (
                                                                                    <td
                                                                                        ...
                                                                                        className={`airring_inspection_party-name-cell ${isLastEntryInParty ? 'no-bottom-border' : ''}`}
                                                                                        style={{
                                                                                    ...
                                                                                    >
                                                                                        <strong
                                                                                            className="airring_inspection_p_span">
                                                                                ...
                                                                                {!shouldPrintWorkOrder && entryIndex === 0 && (
                                                                                    <td
                                                                                        ...
                                                                                        className="airring_inspection_party-name-cells"
                                                                                        style={{
                                                                                ...
                                                                                <td className="gborder">{entry.tolerance}</td>
                                                                                <td className="gborder blank">{entry.a_size}</td>
...
export default AirRingInspectionChart;