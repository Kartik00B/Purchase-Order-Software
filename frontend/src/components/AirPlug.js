...
const AirPlug = ({ entry }) => {
    ...
    const firstInputRef = useRef(null); // Define the reference
    ...
    const [airPlugData, setAirPlugData] = useState([]);
    ...
    const fetchDailyData = async () => {
        try {
            const response = await axios.get("http://localhost:5000/airplug-history");
    ...
    const fetchTolerances = async () => {
        try {
            const response = await axios.get("http://localhost:5000/get-airplug-tolerances", {
                params: {
    ...
    const handleInputChange = (e) => {
        ...
        if (name === "nominal_size" || name === "value_size" || name === "l_size") {
            ...
            if (mmInch === "Inch") {
                ...
                if (!isNaN(numericValue)) {
                    updatedValue = (numericValue * 25.4).toFixed(2); // Convert to mm
                }
            }
            setFormData((prev) => ({
                ...prev,
        ...
        if (name === "air_plug_set" && value !== "") {
            const setValue = parseInt(value);
            setFormData({
                ...formData,
                [name]: `${value} (AP SET)`, // Store in the correct format
                air_plug_ap: setValue,
                setting_ring_sr: setValue * 2,
            });
        } else {
            setFormData({
                ...formData,
    ...
    const validateForm = () => {
        ...
        const excludedFields = ["higher_lower", "remark", "l_size", "value_size", "air_plug_set"];
        ...
        if (validateForm()) {
            try {
                const submissionData = {
                    ...
                    air_plug_ap: Number(formData.air_plug_ap),
                    qty: Number(formData.qty) || 0,
                    sub_sr_no: Number(formData.sub_sr_no) || 0,
                    value_size: Number(formData.value_size) || 0,
                    setting_ring_sr: Number(formData.setting_ring_sr),
                ...
                if (isEditing && editingEntry && editingEntry.record_sr_no === formData.record_sr_no) {
                    await axios.put(`http://localhost:5000/airplug-entry/${submissionData.record_sr_no}`, submissionData);
                    alert("Entry updated successfully!");
                } else {
                    await axios.post("http://localhost:5000/air_plug-entry", submissionData);
                    alert("Entry successfully added to Air Plug table!");
                }
                ...
                setFormData({
                    ...
                    air_plug_ap: formData.air_plug_ap || 0, // Retain previous values
                    setting_ring_sr: formData.setting_ring_sr || 0, // Retain previous values
                    air_plug_set: formData.air_plug_set || "", // Retain previous values
                ...
                firstInputRef.current.focus(); // Set focus back to the first input
                fetchDailyData();
                logUserAction("Make an entry in Air Plug");
            } catch (error) {
    ...
    const handleDeleteAll = async () => {
        if (window.confirm("Are you sure you want to delete ALL Air Plug entries?")) {
            try {
                const response = await axios.delete("http://localhost:5000/delete-all-airplug-entries");

                if (response.data.message.includes("No records found")) {
                    alert("No entries found to delete.");
                } else {
                    setAirPlugData([]);  // Clear state
    ...
    const handleDelete = async (record_sr_no) => {
        if (window.confirm("Are you sure you want to delete this entry? It will be removed from both Air Plug and Purchase Orders tables.")) {
            try {
                const response = await axios.delete(`http://localhost:5000/airplug-entry/${record_sr_no}`);
    ...
    const fetchEntryData = async (recordSrNo) => {
        try {
            // const response = await axios.get(`http://localhost:5000/airplug-entry/${recordSrNo}`);
            if (response.data) {
                ...
                setFormData({
                    ...
                    air_plug_set: entry.air_plug_set,
    ...
    const logUserAction = async (action) => {
        ...
        if (user) {
            try {
                await axios.post("http://localhost:5000/log-action", { userId: user.id, action });
                console.log("Action logged:", action); // Log success for debugging
            } catch (error) {
    ...
    return (
        <div className="air_plug__container">
            <div className="air_plug__form-container">
            <button className="airsnap-back-button"
                ...
                {/* <div className="air_plug-form"> */}
                    <form className="air_plug__form" onSubmit={handleSubmit}>
                        ...
                        <label className="air_snap_label">
                            ...
                            <input
                                ...
                                ref={firstInputRef} // Assign the ref here

                            />
                        </label>
                            <label className="air_plug_label workorder">
                                Work Order No:
                            ...
                            <label className="air_plug_labelp partyname">
                                Party Name:
                                ...
                                >
                                    <option className="air_plug_input" value=""></option>
                            ...
                            <label className="air_plug_label">
                                Purchase Order:
                        ...
                        <div className="form-style2">
                            ...
                            <label className="air_plug_label1">
                                City:
                                {formErrors.city && (
                        ...
                        <div className="form-style3">
                            <label className="air_plug_label1 units">
                                Units:
                                {formErrors.units && (
                                ...
                                <select
                                    ...
                                    style={{color: 'black', fontWeight: 'bold'}}
                                >
                                    <option className="air_plug_input" value="">-- Select the Units --</option>
                        ...
                        <div className="formstyle5">
                            <div className="form-style5">
                                <label className="air_plug_label">
                                    Air Plug:
                                    {formErrors.air_plug_ap && (
                                        <span className="error-message">{formErrors.air_plug_ap}</span>
                                    )}
                                    <br/>
                                    <input
                                        type="text"
                                        name="air_plug_ap"
                                        value={formData.air_plug_ap}
                                        onChange={handleInputChange}
                                        className="air_plug_input"
                                        required
                                        style={{color: 'black', fontWeight: 'bold'}}
                                ...
                                <label className="air_plug_label">
                                    Setting Ring:
                                    {formErrors.setting_ring_sr && (
                                        <span className="error-message">{formErrors.setting_ring_sr}</span>
                                    )}
                                    <br/>
                                    <input
                                        type="text"
                                        name="setting_ring_sr"
                                        value={formData.setting_ring_sr}
                                        onChange={handleInputChange}
                                        className="air_plug_input"
                                        required
                                        style={{color: 'black', fontWeight: 'bold'}}
                                ...
                                <label className="air_plug_label">
    Air Plug Set:
    {formErrors.air_plug_set && (
        <span className="error-message">{formErrors.air_plug_set}</span>
    )}
    <br/>
    <input
        className="air_plug_input"
        ...
        name="air_plug_set"
        style={{color: 'black', fontWeight: 'bold'}}
        value={(formData.air_plug_set || "").replace(" (AP SET)", "")} // Show only numeric value for editing
        onChange={(e) => {
            ...
            setFormData((prevData) => ({
                ...prevData,
                air_plug_set: value ? `${value} (AP SET)` : "", // Append "(AP SET)" when user types
                air_plug_ap: numericValue || 0, // Set air_plug_ap to the numeric value or 0 if invalid
                setting_ring_sr: (numericValue || 0) * 2, // Set setting_ring_sr to double the air_plug_set value
            }));
        }}
    />
</label>
                            ...
                            <div className="form-style6">
                                <div className="formstyle6">
                                    <label className="air_plug_label nominal">
                                    ...
                                    <label className="air_plug_label" style={{marginTop: "27px", marginLeft: "-8px"}}>
                                        <select
                                            type="text"
                                            name="mmToInch"
                                            value={mmInch}
                                ...
                                <div className="form-labelsize">
                                    <label className="air_plug_label slabel" style={{marginTop: "-10px"}}>
                                        Size in labels:
                                        <br/>
                                        <input
                                            type="text"
                                    ...
                                    <label className="air_plug_label vlabel" style={{marginTop: "20px"}}>
                        ...
                        <div className="form-style7">
                            ...
                            <label className="air_plug_label">
                                Remark:
                                <textarea className="air_plug_input" cols={30} rows={4} type="text"
                                          name="remark"
                ...
                {/* </div> */}
                <div className="air_plug__tab-navigation">
                    <button type="submit" onClick={handleSubmit} className="air_plug_button">
                        Submit
                    </button>
                    <Link to="/history" className="air_snap__tab">
                        History
                    </Link>
                </div>
            </div>

            <div className="air_plug__data-container">
                ...
                {/*<button onClick={handlePrint} className="air_plug_button">*/}
                {/*    Print Report*/}
                {/*</button>*/}

                <div className="airplug_table-container" ref={printRef}>
                    <table className="air_plug__table">
                        <thead className="Air_plug_head">
                        <tr>
                            ...
                            return (
                                <tr key={entry.record_sr_no}>
                                    ...
                                    <td>
                                        ...
                                        <br/>
                                        <span className="airplug_party-name">{entry.party_name}</span>
                                        <br/>
                                        <span className="airplug_party-name">{entry.city}</span>
                                    </td>

                                    <td>
                                        <strong>SIZE: {entry.nominal_size.toFixed(4)}</strong>
    ...
    {entry.label_size && entry.label_size !== 'undefinedundefined' ? `${entry.label_size}` : ''}
</strong>                                        <br/>
                                        {
                                            ...
                                            errorMessage ? (
                                                ...
                                                <strong>
                                                    TOL: {size_low_tolerance.toFixed(4)} / {size_high_tolerance.toFixed(4)}
                                                </strong>
                                            )
                                        }
                                        <br/>
