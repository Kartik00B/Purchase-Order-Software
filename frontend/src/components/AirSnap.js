...
const AirSnap = ({entry}) => {
    ...
    const [airsnapUnits, setAirSnapUnits] = useState([]);
...
// Track if the user has manually edited values
    ...
    const [airSnapData, setAirSnapData] = useState([]);
    ...
    const fetchDailyData = async () => {
        try {
            const response = await axios.get("http://localhost:5000/airsnap-history");
    ...
    useEffect(() => {
        axios
            .get("http://localhost:5000/airsnapunits")
            .then((response) => setAirSnapUnits(response.data))
    ...
    const handleInputChange = (e) => {
        ...
        if (name === "air_snap_set" && value !== "") {
            const setValue = parseInt(value);
            setFormData({
                ...formData,
                [name]: `${value} (AS SET)`, // Store in the correct format
                air_snap_as: setValue,
                setting_master_sm: setValue * 2,
            });
        } else {
            setFormData({
                ...formData,
        ...
        if (name === "city") {
            if (cities.includes(value)) {
                ...
                console.log("City error detected:", cityError);
        ...
        if (name === "sunits")
            if (airsnapUnits.includes(value)) {
        ...
        // Special handling for record_sr_no
        if (name === 'record_sr_no') {
            setRecordSrNo(value);
        }
    ...
    const validateForm = () => {
        ...
        const excludedFields = ["higher_lower", "remark", "l_size", "value_size"];
        ...
        if (validateForm()) {
            try {
                const submissionData = {
                    ...
                    air_snap_as: Number(formData.air_snap_as),
                    qty: Number(formData.qty) || 0,
                    sub_sr_no: Number(formData.sub_sr_no) || 0,
                    value_size: Number(formData.value_size) || 0,
                    setting_master_sm: Number(formData.setting_master_sm),
                ...
                if (isEditing && editingEntry && editingEntry.record_sr_no === formData.record_sr_no) {
                    await axios.put(`http://localhost:5000/airsnap-entry/${submissionData.record_sr_no}`, submissionData);
                    alert("Entry updated successfully!");
                } else {
                    await axios.post("http://localhost:5000/air_snap-entry", submissionData);
                    alert("Entry successfully added to Air Snap table!");
                }
                ...
                setFormData({
                    ...
                    air_snap_as: formData.air_snap_as || 0, // Retain previous values
                    setting_master_sm: formData.setting_master_sm || 0, // Retain previous values
                    air_snap_set: formData.air_snap_set || "", // Retain previous values
    ...
    const handleDeleteAll = async () => {
        if (window.confirm("Are you sure you want to delete ALL Air Snap entries?")) {
            try {
                const response = await axios.delete("http://localhost:5000/delete-all-airsnap-entries");
    
                if (response.data.message.includes("No records found")) {
                    alert("No entries found to delete.");
                } else {
                    setAirSnapData([]);  // Clear state
    ...
    const handleDelete = async (record_sr_no) => {
        if (window.confirm("Are you sure you want to delete this entry? It will be removed from both Air Snap and Purchase Orders tables.")) {
            try {
                const response = await axios.delete(`http://localhost:5000/airsnap-entry/${record_sr_no}`);
    ...
    try {
        ...
        if (response.data) {
            ...
            setFormData({
                ...
                qty: entry.qty,
                air_snap_as: entry.air_snap_as,
                setting_master_sm: entry.setting_master_sm,
                air_snap_set: entry.air_snap_set,
...
const handleEdit = (entry) => {
    ...
    return (
        <div className="air_snap__container">
            <div className="air_snap__form-container">
                ...
                <div className="air_snap-form">
                    <form className="air_snap__form" onSubmit={handleSubmit}>
                        ...
                        <label className="air_snap_label">
                            ...
                            <label className="air_snap_label workorder">
                                Work Order No:
                            ...
                            <label className="air_snap_labelp partyname">
                                Party Name:
                                ...
                                >
                                    <option className="air_snap_input" value=""></option>
                        ...
                        <div className="form-style2">
                            ...
                            <label className="air_snap_label1">
                                City:
                                {formErrors.city && (
                        ...
                        <div className="form-style3">
                            <label className="air_snap_label1 units">
                                Units:
                                {formErrors.runits && (
                                ...
                                <select
                                    ...
                                    style={{color: 'black', fontWeight: 'bold'}}
                                >
                                    <option className="air_snap_input" value="">-- Select the Units --</option>
                                    {airsnapUnits.map((sunits, index) => (
                                        <option key={index} value={sunits}>
                                            {sunits}
                                        </option>
                                    ))}
                                </select>
                        ...
                        <div className="formstyle5">
                            <div className="form-style5">
                                <label className="air_snap_label">
                                    Air Snap:
                                    {formErrors.air_snap_as && (
                                        <span className="error-message">{formErrors.air_snap_as}</span>
                                    )}
                                    <br/>
                                    <input
                                        type="text"
                                        name="air_snap_as"
                                        value={formData.air_snap_as}
                                        onChange={handleInputChange}
                                        className="air_snap_input"
                                        required
                                        style={{color: 'black', fontWeight: 'bold'}}
                                ...
                                <label className="air_snap_label">
                                    Setting Master:
                                    {formErrors.setting_master_sm && (
                                        <span className="error-message">{formErrors.setting_master_sm}</span>
                                    )}
                                    <br/>
                                    <input
                                        type="text"
                                        name="setting_ring_sr"
                                        value={formData.setting_master_sm}
                                        onChange={handleInputChange}
                                        className="air_snap_input"
                                        required
                                        style={{color: 'black', fontWeight: 'bold'}}
                                ...
                                <label className="air_snap_label">
                                    Air Snap Set:
                                    {formErrors.air_snap_set && (
                                        <span className="error-message">{formErrors.air_snap_set}</span>
                                    )}
                                    <br/>
                                    <input
                                        className="air_snap_input"
                                        ...
                                        value={formData.air_snap_set.replace(" (AS SET)", "")} // Show only numeric value for editing
                                        onChange={handleInputChange}
                                        onBlur={() => {
                                            // Append "(AP SET)" when user leaves the field
                                            setFormData((prevData) => ({
                                                ...prevData,
                                                air_snap_set: prevData.air_snap_set
                                                    ? `${prevData.air_snap_set.replace(" (AS SET)", "")} (AS SET)`
                                                    : "",
                                            }));
                                        }}
                                    />
                            ...
                            <div className="form-style6">
                                <div className="formstyle6">
                                    <label className="air_snap_label nominal">
                                    ...
                                    <label className="air_snap_label" style={{marginTop: "27px", marginLeft: "-8px"}}>
                                        <select
                                            type="text"
                                            name="mmToInch"
                                            value={mmInch}
                                ...
                                <div className="form-labelsize">
                                    <label className="air_snap_label slabel" style={{marginTop: "-10px"}}>
                                        Size in labels:
                                        <br/>
                                        <input
                                            type="text"
                                    ...
                                    <label className="air_snap_label vlabel" style={{marginTop: "20px"}}>
                        ...
                        <div className="form-style7">
                            <label className="air_snap_label1">
                                ...
                                >
                                    <option disabled value=""></option>
                                    <option selected value="CJ">CJ</option>
                            ...
                            <label className="air_snap_label">
                                Remark:
                                <textarea className="air_snap_input" cols={30} rows={4} type="text"
                                          name="remark"
                ...
                <div className="air_snap__tab-navigation">
                    <button type="submit" onClick={handleSubmit} className="air_snap_button">
                        Submit
                    </button>
            ...
            <div className="air_snap__data-container">
                ...
                <div className="airring_table-container" ref={printRef}>
                    <table className="air_snap__table">
                        <thead className="air_snap_head">
                        <tr>
                            <th>Work Order NO & DATE <br/>
                                PARTY NAME
                            </th>
                            <th className="nominal">NOMINAL SIZE</th>
                            {/*<th>City</th>*/}
                            {/*<th>Units</th>*/}
                            <th>Jet Count</th>
                            <th>Bore/JET Type</th>
